...
namespace HiMentor.SharedKernel.Infrastructure.Email;

/// <summary>
/// Reaproveitado de HiMentor.Infrastructure.Services.Email.EmailService — movido para o
...
) : IEmailService
{
    ...
    private readonly string _from = configuration["Email:From"] ?? "noreply@himentor.com.br";
    private readonly string _fromName = configuration["Email:FromName"] ?? "HiMentor";
    ...
    private readonly string _domain = configuration["Email:Domain"] ?? "himentor.com.br";
    ...
    private readonly string _frontendUrl = configuration["FrontendUrl"] ?? "https://app.himentor.com.br";
    ...
    public async Task SendWelcomeAsync(Guid userId, string to, string firstName, string confirmationToken, CancellationToken ct = default)
    {
        ...
        var body = $"""
            <h2>Olá, {firstName}! Bem-vindo(a) à HiMentor 🎓</h2>
            ...
            """;
        await SendAsync(to, $"Seu código de confirmação HiMentor: {confirmationToken}", body, ct);
    }
    ...
    public async Task SendPasswordResetAsync(string to, string firstName, string resetToken, CancellationToken ct = default)
    {
        ...
        var body = $"""
            <h2>Redefinição de Senha — HiMentor</h2>
            ...
            """;
        await SendAsync(to, "Redefinição de senha — HiMentor", body, ct);
    }
    ...
    public async Task SendPaymentConfirmedAsync(string to, string firstName, decimal amount, CancellationToken ct = default)
    {
        var body = $"""
            ...
            """;
        await SendAsync(to, "Pagamento confirmado — HiMentor", body, ct);
    }
    ...
    public async Task SendCourseAccessGrantedAsync(string to, string firstName, string courseTitle, string courseSlug, CancellationToken ct = default)
    {
        ...
        var body = $"""
            ...
            <p>Se ainda não tem uma conta na HiMentor com este e-mail, crie uma gratuitamente para acessar o conteúdo.</p>
            <a href="{courseUrl}" style="background:#2563EB;color:white;padding:12px 24px;border-radius:6px;text-decoration:none;">
                Acessar o Curso
            </a>
            """;
        await SendAsync(to, $"Acesso liberado: {courseTitle} — HiMentor", body, ct);
    }
    ...
    public async Task SendAccessLinkAsync(string to, string firstName, string magicLinkToken, CancellationToken ct = default)
    {
        ...
        var body = $"""
            <h2>Seu link de acesso — HiMentor</h2>
            ...
            """;
        await SendAsync(to, "Seu link de acesso — HiMentor", body, ct);
    }
