...
using CatalogEntities = HiMentor.Catalog.Domain.Entities;
using LearningEntities = HiMentor.Learning.Domain.Entities;
using JourneyEntities = HiMentor.Journey.Domain.Entities;
using SalesEntities = HiMentor.Sales.Domain.Entities;
using StreamingEntities = HiMentor.Streaming.Domain.Entities;
using FinanceEntities = HiMentor.Finance.Domain.Entities;
using PayoutEntities = HiMentor.Payout.Domain.Entities;
using CreatorStudioEntities = HiMentor.CreatorStudio.Domain.Entities;
using ChannelEntities = HiMentor.Channel.Domain.Entities;
using MediatR;
...
namespace HiMentor.Host.Api.Data;
...
/// Resta só Growth, que é um contexto novo sem código legado (ainda stub). O HiMentor.API antigo
/// (src/HiMentor.API) pode ser desligado depois que os dois lados forem validados lado a lado —
/// ver Task #18.
/// Os dois hosts apontam para bancos diferentes (himentor_dev vs himentor_modular_dev); não rode os
...
) : DbContext(options), IUnitOfWork, IDataProtectionKeyContext
{
    ...
    /// <summary>
    /// Ver IUnitOfWork.TrySaveChangesAsync — usado por fluxos onde duas requisições concorrentes
    /// podem legitimamente disputar a inserção do mesmo registro (ex.: SyncCreatorOnboardingDocuments,
    /// dois GET simultâneos descobrindo o mesmo documento novo da Asaas ao mesmo tempo). Só
    /// engole a exceção quando a causa raiz é comprovadamente uma violação de restrição única do
    /// Postgres (SQLSTATE 23505) — qualquer outro tipo de DbUpdateException (inclusive
    /// DbUpdateConcurrencyException por outro motivo) continua sendo propagado.
    /// </summary>
    public async Task<bool> TrySaveChangesAsync(CancellationToken ct = default)
    {
        ...
        catch (DbUpdateException ex) when (IsUniqueConstraintViolation(ex))
        {
            logger.LogInformation(ex,
                "SaveChanges perdeu uma corrida de inserção concorrente (violação de restrição única) — " +
                "descartando rastreamento desta unidade de trabalho para nova tentativa com estado atualizado.");
            ChangeTracker.Clear();
            return false;
        }
    }
