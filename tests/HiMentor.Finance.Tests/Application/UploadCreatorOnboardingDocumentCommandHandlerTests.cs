...
using HiMentor.Finance.Tests.Fakes;
using Xunit;

namespace HiMentor.Finance.Tests.Application;
...
public class UploadCreatorOnboardingDocumentCommandHandlerTests
{
        ...
        string asaasDocumentId, string? onboardingUrl = null)
    {
        ...
        subaccount.SyncDocuments([(asaasDocumentId, "IDENTIFICATION", "RG", null, HiMentor.Finance.Domain.Enums.OnboardingDocumentStatus.Pending, onboardingUrl)]);
    ...
    public async Task Handle_SuccessfulUpload_ResyncsAndUpdatesExistingDocument_WithoutTreatingItAsNew()
    {
        ...
        Assert.Equal(HiMentor.Finance.Domain.Enums.OnboardingDocumentStatus.AwaitingApproval, subaccount!.Documents.Single(d => d.AsaasDocumentId == "doc_1").Status);
    }
