using HiMentor.Catalog.Application.Commands.AddLesson;
using HiMentor.Catalog.Application.Commands.AddLessonAttachment;
using HiMentor.Catalog.Application.Commands.AddModule;
using HiMentor.Catalog.Application.Commands.ArchiveCourse;
using HiMentor.Catalog.Application.Commands.CreateCourse;
using HiMentor.Catalog.Application.Commands.DeleteCourse;
using HiMentor.Catalog.Application.Commands.DuplicateCourse;
using HiMentor.Catalog.Application.Commands.PublishCourse;
using HiMentor.Catalog.Application.Commands.RecordCourseView;
using HiMentor.Catalog.Application.Commands.ReorderLessons;
using HiMentor.Catalog.Application.Commands.ReorderModules;
using HiMentor.Catalog.Application.Commands.SetCoursePrice;
using HiMentor.Catalog.Application.Commands.SetCourseSalesPage;
using HiMentor.Catalog.Application.Commands.UpdateCourseBasicInfo;
using HiMentor.Catalog.Application.Commands.UpdateLesson;
using HiMentor.Catalog.Application.Queries.GetCourseBySlug;
using HiMentor.Catalog.Application.Queries.GetOtherCoursesByInstructor;
using HiMentor.Catalog.Application.Queries.ListCourses;
...
/// HiMentor.Streaming.Api.Controllers.LessonPlaybackController (mesma rota, para não quebrar
...
public sealed class CoursesController(ISender sender, ICurrentUserService currentUser) : ControllerBase
{
    ...
    /// <summary>Edita título/descrição de uma aula já criada (passo 3 do assistente — usado para
    /// preencher a descrição depois que a aula já existe, ex.: já tem vídeo vinculado).</summary>
    [HttpPut("{courseId:guid}/modules/{moduleId:guid}/lessons/{lessonId:guid}")]
    [Authorize(Roles = "Creator,Admin")]
    public async Task<IActionResult> UpdateLesson(Guid courseId, Guid moduleId, Guid lessonId,
        [FromBody] UpdateLessonCommand command, CancellationToken ct)
    {
        await sender.Send(
            command with { CourseId = courseId, ModuleId = moduleId, LessonId = lessonId, InstructorId = currentUser.UserId!.Value }, ct);
        return Ok(new { message = "Aula atualizada com sucesso." });
    }
