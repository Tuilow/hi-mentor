using HiMentor.SharedKernel.Application.Exceptions;
using HiMentor.SharedKernel.Application.Interfaces;
using HiMentor.Catalog.Domain.Interfaces;
using MediatR;

namespace HiMentor.Catalog.Application.Commands.SetCoursePrice;

public sealed class SetCoursePriceCommandHandler(
    ICourseRepository courseRepository, IUnitOfWork uow
) : IRequestHandler<SetCoursePriceCommand>
{
    public async Task Handle(SetCoursePriceCommand request, CancellationToken ct)
    {
        var course = await courseRepository.GetByIdAsync(request.CourseId, ct)
            ?? throw new NotFoundException("Curso", request.CourseId);

        if (course.InstructorId != request.InstructorId)
            throw new ForbiddenException("Apenas o criador pode definir o preço deste produto.");

        course.SetPrice(request.Price);
        courseRepository.Update(course);
        await uow.SaveChangesAsync(ct);
    }
}
