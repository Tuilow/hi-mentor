using HiMentor.Streaming.Application.Commands.DeleteVideo;
using HiMentor.Streaming.Application.Commands.GetVideoUploadUrl;
using HiMentor.Streaming.Application.Commands.ImportExternalVideo;
using HiMentor.Streaming.Application.Commands.LinkVideoToLesson;
using HiMentor.Streaming.Application.Queries.GetVideosByCourse;
using HiMentor.SharedKernel.Application.Interfaces;
using MediatR;
using Microsoft.AspNetCore.Authorization;
using Microsoft.AspNetCore.Mvc;

namespace HiMentor.Streaming.Api.Controllers;
...
public sealed class VideosController(ISender sender, ICurrentUserService currentUser) : ControllerBase
{
    ...
    {
        var result = await sender.Send(new GetVideoUploadUrlCommand(request.CourseId, currentUser.UserId!.Value, request.Title), ct);
        return Ok(result);
    }
...
public sealed record GetUploadUrlRequest(Guid CourseId, string? Title = null);
