...
public sealed class CreatorAsaasSubaccountRepository(DbContext context) : ICreatorAsaasSubaccountRepository
{
    ...
    public async Task AddDocumentAsync(CreatorAsaasOnboardingDocument document, CancellationToken ct = default) =>
        await context.Set<CreatorAsaasOnboardingDocument>().AddAsync(document, ct);
}
