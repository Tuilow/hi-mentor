...
///   - CreateSubaccountAsync usa a credencial da CONTA PAI da HiMentor ("Asaas:ApiKey" — o mesmo
...
) : IAsaasSubaccountClient
{
    ...
    public async Task<CreateAsaasSubaccountResult> CreateSubaccountAsync(CreateAsaasSubaccountRequest request, CancellationToken ct = default)
    {
        try
        {
            ...
            if (string.IsNullOrWhiteSpace(parentApiKey))
                throw new InvalidOperationException("Asaas:ApiKey (conta pai da HiMentor) não configurado — obrigatório para criar subcontas via BaaS.");
    ...
    public async Task<bool> RegisterAccountStatusWebhookAsync(string subaccountApiKeyPlaintext, string webhookToken, CancellationToken ct = default)
    {
        ...
        return await AsaasWebhookRegistrar.RegisterOrUpdateAsync(
            client, webhookUrl, "HiMentor - Onboarding Financeiro", notificationEmail, webhookToken, events, logger, ct);
    }

    // ─── Webhook de pagamento (marketplace de split, credencial da subconta) ───

    public async Task<bool> RegisterPaymentWebhookAsync(string subaccountApiKeyPlaintext, string webhookToken, CancellationToken ct = default)
    {
        using var client = CreateClient(subaccountApiKeyPlaintext);

        // Reaproveita a MESMA url já usada pelo modelo legado (AsaasAccountOnboardingService.RegisterWebhookAsync)
        // -- aponta para Sales.AsaasWebhookController (api/v1/webhooks/asaas), que já sabe
        // autenticar contra CreatorAsaasSubaccount.WebhookTokenHash (ver AsaasWebhookAuthenticator).
        // Nenhuma variável de ambiente nova é necessária.
        var webhookUrl = configuration["Asaas:CreatorWebhookUrl"];
        if (string.IsNullOrWhiteSpace(webhookUrl))
        {
            logger.LogError("Asaas:CreatorWebhookUrl não configurado — não é possível registrar o webhook de pagamento na subconta.");
            return false;
        }
        ...
        // Mesmos eventos do modelo legado (AsaasAccountOnboardingService.RegisterWebhookAsync) --
        // inclui os eventos de divergência de split, tratados em ProcessAsaasWebhookCommandHandler.
        string[] events =
        [
        ...
        return await AsaasWebhookRegistrar.RegisterOrUpdateAsync(
            client, webhookUrl, "HiMentor - Marketplace (Subconta)", notificationEmail, webhookToken, events, logger, ct);
    }
