using HiMentor.Finance.Application.Commands.StartCreatorFinancialOnboarding;
using HiMentor.Finance.Application.Commands.SyncCreatorOnboardingAccountStatus;
using HiMentor.Finance.Application.Commands.SyncCreatorOnboardingDocuments;
using HiMentor.Finance.Application.Commands.UploadCreatorOnboardingDocument;
using HiMentor.Finance.Application.Queries.GetMyFinancialOnboardingStatus;
...
/// Asaas (BaaS) criada pela própria HiMentor. Substitui, para criadores novos, o fluxo de "cole sua
...
public sealed class CreatorFinancialOnboardingController(
    ISender sender, ICurrentUserService currentUser, ILogger<CreatorFinancialOnboardingController> logger
) : ControllerBase
{
    /// <summary>
    /// Status atual da jornada de onboarding financeiro do criador autenticado. Antes de ler,
    /// dispara um refresh best-effort do veredito geral direto na Asaas (ver
    /// SyncCreatorOnboardingAccountStatusCommand) — rede de segurança para quando o webhook
    /// ACCOUNT_STATUS_* não chega (ver racional completo no comando). Uma falha nesse refresh
    /// (rede, Asaas fora do ar) nunca deve derrubar a tela de status — por isso o try/catch aqui:
    /// é enriquecimento best-effort de uma leitura, não uma operação crítica.
    /// </summary>
    [HttpGet]
    public async Task<IActionResult> GetStatus(CancellationToken ct)
    {
        try
        {
            await sender.Send(new SyncCreatorOnboardingAccountStatusCommand(currentUser.UserId!.Value), ct);
        }
        catch (Exception ex)
        {
            logger.LogWarning(ex, "Refresh best-effort de status de conta falhou para o criador {CreatorId} -- prosseguindo com o status já persistido.", currentUser.UserId);
        }
    ...
    /// <summary>Envio de um documento pela própria HiMentor — só funciona para documentos sem link externo obrigatório (ver CreatorAsaasOnboardingDocument.OnboardingUrl).</summary>
