...
/// final desta feature).
///
/// IMPORTANTE -- por que isto NAO e uma "subconta" criada pela HiMentor via API (POST
...
/// HiMentor sao autonomos sem CNPJ, o modelo escolhido (decisao do dono do produto) foi: o proprio
/// creator cria/ja possui uma conta Asaas comum (pessoa fisica ou juridica, fora da API de
/// subcontas), gera uma API Key nessa conta e conecta aqui. A HiMentor usa essa API Key para criar
/// a cobranca DIRETAMENTE na conta do creator (ele e o emissor/vendedor de fato, inclusive para
/// fins de nota fiscal) com um split apontando para a walletId da HiMentor (comissao). Isso nao
...
public sealed class CreatorAsaasAccount : AggregateRoot
{
    ...
    /// <summary>walletId da conta do creator -- usado apenas informativamente aqui (o split aponta para a walletId da HIMENTOR, nao esta).</summary>
    ...
    /// <summary>Hash (SHA-256) do token de webhook que a HiMentor gerou e registrou na conta do creator -- usado para autenticar webhooks recebidos sem precisar decriptar nada (ver AsaasWebhookController).</summary>
