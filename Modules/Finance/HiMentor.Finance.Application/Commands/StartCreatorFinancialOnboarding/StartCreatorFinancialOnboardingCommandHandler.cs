...
///      conta mas a resposta nunca chegou ao HiMentor: recuperação nesse caso é manual, via suporte
...
) : IRequestHandler<StartCreatorFinancialOnboardingCommand, StartCreatorFinancialOnboardingResult>
{
    public async Task<StartCreatorFinancialOnboardingResult> Handle(StartCreatorFinancialOnboardingCommand request, CancellationToken ct)
    {
        ...
        if (!webhookRegistered)
        {
            ...
            // devemos tentar criar outra. Loga como crítico para acompanhamento manual e segue o
            // onboarding normalmente (o criador não deve ficar travado por causa disso) — ver
            // SyncCreatorOnboardingAccountStatusCommand para a rede de segurança que reconsulta o
            // status diretamente caso este webhook nunca chegue.
            logger.LogCritical(
        ...
        // Webhook de PAGAMENTO -- registrado separadamente do de status de conta acima (url e
        // eventos diferentes, mesmo token). Sem isso, uma compra marketplace na conta deste
        // criador nunca recebe confirmação de volta da Asaas (ver Sales.AsaasWebhookController /
        // AsaasMarketplacePaymentService) -- fica presa em "Pending" para sempre. Mesma postura:
        // best-effort, nunca trava o onboarding, log crítico para acompanhamento manual.
        var paymentWebhookRegistered = await asaasSubaccountClient.RegisterPaymentWebhookAsync(creation.ApiKey!, webhookToken, ct);
        if (!paymentWebhookRegistered)
        {
            // Mesma postura do de status acima: best-effort, nunca trava o onboarding. Fica
            // PaymentWebhookRegisteredAt == null -- SyncCreatorOnboardingAccountStatusCommandHandler
            // tenta de novo (rotacionando o token) na próxima vez que este criador carregar a
            // tela de status, até conseguir.
            logger.LogCritical(
                "Subconta Asaas {AccountId} criada para o criador {CreatorId}, mas o registro do webhook de PAGAMENTO falhou — " +
                "compras no marketplace deste criador não receberão confirmação (PAYMENT_*) até isso ser corrigido.",
                creation.AsaasAccountId, request.CreatorId);
        }
        else
        {
