...
const planIcon = (name: string) => {
  ...
  return <Star className="w-5 h-5 text-zinc-400" />;
};
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="relative w-full max-w-md bg-zinc-900 border border-zinc-800 rounded-2xl shadow-2xl animate-fade-in">
        ...
        <div className="flex items-center justify-between p-6 border-b border-zinc-800">
          <div>
            <h2 className="text-lg font-bold text-zinc-100">Finalizar assinatura</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          {/* Trial notice */}
          {plan.trialDays > 0 && (
            <div className="flex items-start gap-2 p-3 bg-emerald-950/40 border border-emerald-800/40 rounded-lg">
          ...
          <div className="flex items-center gap-3 py-2">
            ...
            <span className="text-xs font-medium text-zinc-300 bg-zinc-800 px-2 py-0.5 rounded">PIX</span>
            <span className="text-xs font-medium text-zinc-300 bg-zinc-800 px-2 py-0.5 rounded">Cartão</span>
            <span className="text-xs font-medium text-zinc-300 bg-zinc-800 px-2 py-0.5 rounded">Boleto</span>
          </div>
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      {subscription?.isActive && (
        <div className="card border-emerald-800/40 bg-emerald-950/20 mb-8">
          <div className="flex items-start justify-between gap-4">
            <div>
              <div className="flex items-center gap-2 mb-1">
                <span className="inline-flex items-center gap-1 text-xs font-medium text-emerald-400
                                 bg-emerald-950/60 border border-emerald-800/50 px-2 py-0.5 rounded-full">
      ...
      {loadingPlans ? (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {[1, 2, 3].map(i => (
            <div key={i} className="card border-zinc-800 animate-pulse h-80" />
          ))}
        </div>
      ) : plans.length === 0 ? (
        <div className="card border-zinc-800 text-center py-12">
          <p className="text-zinc-400">Nenhum plano disponível no momento.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {plans.map((plan) => {
            ...
            return (
              <div
                ...
                className={`relative flex flex-col rounded-2xl border p-6 transition-all duration-200
                  ${highlighted
                    ? 'border-brand-600 bg-brand-950/20 shadow-lg shadow-brand-900/20'
              ...
              >
                ...
                {plan.description && (
                  <p className="text-sm text-zinc-400 mb-4 leading-relaxed">{plan.description}</p>
                )}

                {/* Price */}
                <div className="mb-1">
                  <span className={`text-3xl font-extrabold ${highlighted ? 'gradient-text' : 'text-zinc-100'}`}>
                    R$ {plan.price.toFixed(2).replace('.', ',')}
                  </span>
                  <span className="text-sm text-zinc-500 ml-1">{cycleLabel[plan.billingCycle]}</span>
                </div>
                ...
                <ul className="space-y-2.5 my-5 flex-1">
                  {plan.features.map((f) => (
                    <li key={f.featureKey} className="flex items-start gap-2 text-sm text-zinc-300">
                      <Check className={`w-4 h-4 mt-0.5 flex-shrink-0 ${highlighted ? 'text-brand-400' : 'text-zinc-500'}`} />
      ...
      {!subscription?.isActive && (
        <div className="mt-10 grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            ...
            <div key={q} className="p-4 rounded-xl border border-zinc-800 bg-zinc-900/50">
