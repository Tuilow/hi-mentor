...
function Badge({ children, color = 'zinc' }: { children: React.ReactNode; color?: string }) {
  const colors: Record<string, string> = {
    ...
    brand:  'bg-brand-950/60 text-brand-400 border border-brand-800/40',
    ...
    <div className="space-y-3">
      ...
      <div
        ...
        className={`border-2 border-dashed rounded-xl p-6 text-center transition-colors cursor-pointer
          ${file ? 'border-brand-700 bg-brand-950/20' : 'border-zinc-700 hover:border-zinc-600'}
      ...
      >
        ...
        {file ? (
          ...
          <>
            <p className="text-sm text-zinc-400">Arraste o vídeo ou clique para selecionar</p>
            <p className="text-xs text-zinc-600 mt-1">MP4, MOV, WebM — máx. 1 hora</p>
          </>
        )}
      </div>
      ...
      <label className="flex items-center gap-2 cursor-pointer select-none">
        <div
          ...
          className={`w-9 h-5 rounded-full transition-colors relative ${isPreview ? 'bg-brand-600' : 'bg-zinc-700'}`}
        >
        ...
        <span className="text-sm text-zinc-300">
          {isPreview
            ? <><Eye className="inline w-3.5 h-3.5 mr-1 text-brand-400" />Preview gratuito</>
            : <><EyeOff className="inline w-3.5 h-3.5 mr-1 text-zinc-500" />Requer assinatura</>
          }
        </span>
      </label>
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-zinc-100">Gerenciar Cursos</h1>
          <p className="text-zinc-400 mt-1 text-sm">Crie cursos, módulos, aulas e faça upload das videoaulas.</p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="card border-zinc-800 text-center py-12">
          ...
          <p className="text-zinc-400">Nenhum curso ainda. Crie o primeiro!</p>
        </div>
      ) : (
        <div className="space-y-3">
          {courses.map(course => (
            <div key={course.id} className="border border-zinc-800 rounded-xl overflow-hidden bg-zinc-900">
              {/* Course header */}
              <div className="flex items-center gap-3 p-4">
                ...
                <button
                  ...
                  className="text-xs text-brand-400 hover:text-brand-300 font-medium transition-colors px-3 py-1 border border-brand-800/40 rounded-lg"
                >
                  Publicar
                </button>
              </div>
              ...
              {expandedCourse === course.id && courseDetail && (
                <div className="border-t border-zinc-800 bg-zinc-950/40 p-4 space-y-3">
                  ...
                  {(courseDetail.modules ?? []).map((mod: ModuleItem) => (
                    <div key={mod.id} className="border border-zinc-800 rounded-lg overflow-hidden">
                      <div className="flex items-center gap-2 p-3 bg-zinc-900/60">
                        <button
                        ...
                        >
                          ...
                          <span className="text-sm font-medium text-zinc-200">{mod.title}</span>
                        </button>
                        <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-md bg-zinc-900 border border-zinc-800 rounded-2xl shadow-2xl p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-bold text-zinc-100">Upload de vídeo</h2>
              <button onClick={() => setUploadTarget(null)} className="text-zinc-500 hover:text-zinc-300">
                <X className="w-5 h-5" />
              </button>
            </div>
            <VideoUploader
...
function CreateCourseForm({ onSubmit, onCancel, loading }: {
  ...
  return (
    <div className="card border-brand-800/40 bg-brand-950/10 mb-6">
      <h3 className="font-semibold text-zinc-100 mb-4">Novo curso</h3>
      <div className="space-y-3">
        ...
        <div className="flex gap-2 pt-1">
          ...
          <button onClick={onCancel} className="px-4 py-2 text-sm text-zinc-400 hover:text-zinc-200 border border-zinc-700 rounded-lg">
            Cancelar
          </button>
        </div>
      </div>
...
function AddModuleForm({ onSubmit, onCancel, loading }: {
  ...
  return (
    <div className="flex gap-2 items-end mt-1">
      ...
      <button onClick={onCancel} className="px-3 py-2 text-sm text-zinc-500 hover:text-zinc-300">Cancelar</button>
    </div>
  );
}
  ...
  return (
    <div className="space-y-2">
      ...
      <label className="flex items-center gap-2 cursor-pointer select-none">
        ...
        <span className="text-xs text-zinc-400">Preview gratuito (visível sem assinatura)</span>
      </label>
      <div className="flex gap-2">
        <button onClick={() => onSubmit({ title, isPreview })} disabled={!title || loading} className="btn-primary text-sm px-3 py-1.5">
          {loading ? '...' : 'Criar aula'}
        </button>
        <button onClick={onCancel} className="text-sm text-zinc-500 hover:text-zinc-300">Cancelar</button>
      </div>
    </div>
  );
}
