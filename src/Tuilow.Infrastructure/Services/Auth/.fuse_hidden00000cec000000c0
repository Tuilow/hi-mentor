...
namespace DogMaster.Infrastructure.Services.Auth;

public sealed class JwtService(IConfiguration configuration) : IJwtService
{
    private readonly string _secret = configuration["Jwt:Secret"]
        ?? throw new InvalidOperationException("Jwt:Secret não configurado.");
    private readonly string _issuer = configuration["Jwt:Issuer"] ?? "DogMasterPro";
    private readonly string _audience = configuration["Jwt:Audience"] ?? "DogMasterPro";
