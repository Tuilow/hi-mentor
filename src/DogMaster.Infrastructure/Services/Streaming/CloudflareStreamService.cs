...
) : IStreamingService
{
    ...
    private readonly string _streamSigningKey =
        configuration["Cloudflare:StreamSigningKey"] ?? "";
    ...
    {
        // Sem signing key → modo dev, usa URL pública
        ...
        var token = new JwtSecurityToken(
            claims: [
                ...
                new Claim("exp", DateTimeOffset.UtcNow.AddMinutes(expirationMinutes)
                    .ToUnixTimeSeconds().ToString()),
                new Claim("nbf", DateTimeOffset.UtcNow.AddMinutes(-1)
                    .ToUnixTimeSeconds().ToString()),
        ...
        return Task.FromResult(
            $"https://iframe.cloudflarestream.com/{cloudflareVideoId}?token={jwt}");
    }
