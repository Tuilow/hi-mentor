...
// GET /certificates/me e GET /certificates/course/{courseId} — certificado emitido
// automaticamente quando um curso é concluído 100% (ver CourseCompletedEventHandler no
// backend). Alimenta a aba "Certificados" do sidebar e o botão "Baixar certificado" na tela
// de jornada (MentoradoJourneyView).
...
export interface LessonAttachmentDetail {
  id: string;
  title: string;
  fileUrl: string;
  fileType?: string;
...
export interface LessonDetail {
  ...
  isPreview: boolean;
  hasVideo: boolean;
  attachments?: LessonAttachmentDetail[];
}
...
// ─── Código de acesso ─────────────────────────
// Ativação pelo aluno (bloco "Tenho um código de acesso" no dashboard sem programas) e emissão
// pelo painel Admin ("Plataforma" → "Códigos de acesso").

// POST /enrollments/redeem-code
...
export interface AccessCodeAdminItem {
  id: string;
  code: string;
  courseId: string;
  courseTitle: string;
