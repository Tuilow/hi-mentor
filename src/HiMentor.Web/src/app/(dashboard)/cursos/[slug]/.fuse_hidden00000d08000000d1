  ...
  if (!course) {
    return (
      <div className="text-center py-20 text-zinc-500">
        ...
        <Link href="/cursos" className="text-brand-400 hover:underline text-sm mt-2 inline-block">
          ← Voltar para cursos
        </Link>
      </div>
    );
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-2 text-sm text-zinc-500 mb-6">
        <Link href="/cursos" className="hover:text-zinc-300 transition-colors">Cursos</Link>
        <span>›</span>
        <span className="text-zinc-300 truncate">{course.title}</span>
      </div>

      {/* Hero */}
      <div className="card border-zinc-800 mb-6 overflow-hidden p-0">
        {course.thumbnailUrl ? (
          <img src={course.thumbnailUrl} alt={course.title}
            className="w-full h-56 object-cover" />
        ) : (
          <div className="w-full h-56 bg-gradient-to-br from-zinc-800 to-zinc-900
        ...
        <div className="p-6">
          ...
          <h1 className="text-2xl font-bold text-zinc-100 mb-2">{course.title}</h1>
          {course.description && (
            <p className="text-zinc-400 text-sm leading-relaxed mb-4">{course.description}</p>
          )}

          <div className="flex flex-wrap items-center gap-6 pt-4 border-t border-zinc-800">
            <div className="text-center">
              <p className="text-lg font-bold text-zinc-100">{totalLessons}</p>
              <p className="text-xs text-zinc-500">Aulas</p>
            </div>
            <div className="text-center">
              <p className="text-lg font-bold text-zinc-100">{course.modules.length}</p>
              <p className="text-xs text-zinc-500">Módulos</p>
            </div>
            {course.totalDurationMinutes && (
              <div className="text-center">
                <p className="text-lg font-bold text-zinc-100">{course.totalDurationMinutes}min</p>
      ...
      {/* Conteúdo do curso */}
      <div>
        <h2 className="text-lg font-semibold text-zinc-100 mb-4">Conteúdo do curso</h2>
        <div className="space-y-2">
          {course.modules
            ...
            .map(module => {
              ...
              return (
                <div key={module.id} className="card border-zinc-800 p-0 overflow-hidden">
                  {/* Módulo header */}
                  <button
                      ...
                      {module.lessons
                        ...
                        .map((lesson, idx) => (
                          <div key={lesson.id}
                            ...
                            <span className="text-xs text-zinc-600 w-5 flex-shrink-0">
                              {idx + 1}
                            </span>
                            ...
                            {lesson.isPreview && lesson.hasVideo && (
                              <Link
                                ...
                                className="text-xs text-brand-400 font-medium
                                           ...
                                           transition-opacity hover:text-brand-300"
                              >
                                Assistir →
                              </Link>
                            )}
                            {!lesson.isPreview && (
                              <span className="text-xs text-zinc-600">Assinantes</span>
                            )}
                          </div>
                        ))}
                    </div>
