...
import MentoradoJourneyView from './MentoradoJourneyView';
...
// Exportadas (Lesson/Module/CourseDetail/EnrollmentProgressDto/LessonProgressDto) para reuso em
// ./MentoradoJourneyView.tsx — mesmos shapes, sem duplicar declaração.
...
export interface CourseDetail {
  ...
  modules: Module[];
  instructorId: string;
  // Redesign "Hi Mentor" (tela de jornada do mentorado, ago/2026): já vêm de
  // GetCourseBySlugQueryHandler (CourseDetailResponse) hoje — só não estavam declarados aqui
  // porque esta página não os usava ainda. Alimentam "com {mentor}" no hero da nova tela.
...
export interface EnrollmentProgressDto {
  ...
  // Redesign "Hi Mentor" (tela de jornada do mentorado, ago/2026): GetEnrollmentProgressQueryHandler
  // (EnrollmentProgressResponse) já devolve o progresso por aula hoje — só não estava declarado
  // aqui porque esta página não usava granularidade de aula antes. Alimenta os estados
  // done/current/available do accordion "Sua jornada" e o card "Você parou em".
...
export default function CourseDetailPage() {
  ...
  // HiMentor.Sales.Domain.Entities.Plan.CourseId e SubscriptionRepository.GetPlansByCourseAsync.
  ...
  // Redesign "Hi Mentor" (ago/2026): quando o aluno já está matriculado, a tela de "vitrine do
  // curso" (CTA de compra, checkout, etc.) não faz mais sentido — ele já pagou. Troca inteira
  // pela tela de jornada (MentoradoHome do protótipo), mantendo a mesma URL/rota. Quem NÃO está
  // matriculado (visitante ou o próprio mentor dono do curso — instrutores não têm Enrollment do
  // próprio curso) continua vendo a tela abaixo sem nenhuma mudança.
  if (isEnrolled && enrollment) {
    return (
      <MentoradoJourneyView
        slug={slug as string}
        course={course}
