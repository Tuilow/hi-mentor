...
import type { CreatorFinancialOnboardingStatus } from '@/lib/api';
...
// (título/copy) ao restante do assistente. Ver HiMentor.Catalog.Domain.Enums.ProductType.
...
function ProductWizard() {
  ...
  // Aviso não-bloqueante no passo de Publicação (7) sobre o onboarding financeiro (subconta
  // Asaas/BaaS) -- o bloqueio de verdade acontece no backend (PublishProductCommandHandler); isto
  // aqui é só pra o criador não ser surpreendido só depois de clicar em "Publicar Produto". Só
  // busca quando o criador chega no passo 7 (sem custo nos passos anteriores).
  const { data: financialStatus } = useQuery<CreatorFinancialOnboardingStatus>({
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <p className="text-gray-500 text-sm mb-6">
        Publique seu produto em poucos passos — a HiMentor não cobra mensalidade, só uma comissão sobre as vendas.
      </p>
      ...
      <div className="card">
        ...
        {step === 7 && (
          <div className="space-y-5">
            ...
            {financialStatus && !financialStatus.canSell && (
              <div className="flex items-start gap-2 p-3 rounded-lg bg-orange-50 border border-orange-200 text-orange-800 text-sm">
                ...
                <div>
                  <p>Falta configurar seus recebimentos antes de publicar — a venda fica bloqueada até isso ser concluído.</p>
                  <a href="/admin/financeiro" className="underline font-medium">Configurar recebimentos</a>
                </div>
              </div>
            )}
            {checklist ? (
