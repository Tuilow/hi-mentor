...
namespace Tuilow.Finance.Tests.Security;
...
/// Tuilow.Finance.Application.Commands/Queries.
///
/// Escopo deliberadamente EXCLUI:
/// - Tuilow.Finance.Application.Interfaces (IAsaasSubaccountClient e seus DTOs, ex.
...
///   direção que importa: o que a Tuilow devolve para fora, nunca o que o usuário manda pra
...
public class NoSecretsInResponseDtosTests
{
    ...
    {
        var assembly = typeof(Tuilow.Finance.Application.Commands.StartCreatorFinancialOnboarding.StartCreatorFinancialOnboardingCommand).Assembly;

        var responseTypes = assembly.GetTypes()
            .Where(t => t.IsPublic)
            .Where(t => t.Namespace is not null &&
                        (t.Namespace.StartsWith("Tuilow.Finance.Application.Commands", StringComparison.Ordinal) ||
                         t.Namespace.StartsWith("Tuilow.Finance.Application.Queries", StringComparison.Ordinal)))
            .Where(t => !t.Namespace!.StartsWith("Tuilow.Finance.Application.Interfaces", StringComparison.Ordinal))
